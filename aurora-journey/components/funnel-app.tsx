...
export function FunnelApp({ funnelId = defaultFunnelId }: { funnelId?: string }) {
  ...
  useEffect(() => { initBrowserProviders(); }, []);
  useEffect(() => { if (!hasViewedFunnel.current) { hasViewedFunnel.current = true; void track("funnel_viewed", { ...context, eventId: crypto.randomUUID() }); } }, [context]);
  useEffect(() => {
    if (viewedScreens.current.has(current.id)) return;
    viewedScreens.current.add(current.id);
    void track("screen_viewed", { ...context, eventId: crypto.randomUUID() });
  ...
  const setAnswer = (field: string, value: string | number | boolean) => {
    ...
    if (current.type === "quiz_question" && field !== "dob") void track("quiz_question_answered", { ...context, eventId: crypto.randomUUID() }, { field, value });
  };
