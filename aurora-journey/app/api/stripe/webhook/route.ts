...
export async function POST(request: Request) {
  ...
  const event: TrackedEvent = {
    ...
    properties: {
      ...
      value: checkout.amount_total ? checkout.amount_total / 100 : undefined,
      currency: checkout.currency,
    },
    context: {
      eventId: `stripe_${stripeEvent.id}`,
      occurredAt: new Date(stripeEvent.created * 1_000).toISOString(),
      productId: metadata.product_id ?? "unknown",
      funnelId: metadata.funnel_id ?? "unknown",
      funnelVersion: "server",
      sessionId: metadata.funnel_session_id ?? checkout.id,
      assignments: metadata.experiment_assignments ? JSON.parse(metadata.experiment_assignments) : {},
      attribution: metadata.attribution ? JSON.parse(metadata.attribution) : { firstTouch: {}, currentTouch: {} },
      identity: { email: checkout.customer_details?.email ?? undefined },
    },
  };
  await serverAnalytics.deliver(event, request);
