...
Keep build tokens and runtime credentials in Secret Manager. Configure `NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY` for the web build and `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, `SHIPFLOW_PUBLIC_URL`, and each `STRIPE_PRICE_*` value at runtime. Register the production domain as a Stripe payment-method domain when wallets are enabled. Product backends consume Stripe directly. `SHIPFLOW_PRODUCT_WEBHOOK_URL` and `SHIPFLOW_PRODUCT_WEBHOOK_SECRET` enable the advisory signed Shipflow relay. Run `pnpm validate`, `pnpm test`, `pnpm build`, and deployment doctor before handoff.
