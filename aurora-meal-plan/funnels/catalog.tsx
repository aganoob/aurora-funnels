...
function BackIcon() {
  return <svg aria-hidden="true" viewBox="0 0 24 24"><path d="m15 4-8 8 8 8" /></svg>;
}
...
function FlowHeader({ progress, previous, close }: { progress: number; previous: () => void; close?: () => void }) {
  return <header className="aurora-flow-header">
    <button className="aurora-icon-button" aria-label="Go back" onClick={previous}><BackIcon /></button>
    ...
    {close ? <button className="aurora-icon-button aurora-close-button" aria-label="Skip this introduction" onClick={close}><CloseIcon /></button> : <span className="aurora-wordmark" aria-label="Aurora">A</span>}
  </header>;
}

function Laurel({ side }: { side: "left" | "right" }) {
  return <img className={`aurora-laurel aurora-laurel-${side}`} src={`/assets/laurel-${side}.svg`} alt="" />;
}

function AppStoreIcon() {
  return <svg className="aurora-source-icon aurora-app-store-icon" aria-hidden="true" viewBox="0 0 32 32">
    <defs><linearGradient id="app-store-blue" x1="4" y1="2" x2="28" y2="30" gradientUnits="userSpaceOnUse"><stop stopColor="#35b9ff" /><stop offset="1" stopColor="#0878ee" /></linearGradient></defs>
    <rect width="32" height="32" rx="7" fill="url(#app-store-blue)" />
    <path d="m11.5 23 8.8-15.2M9 19h14.2M14.5 8l9 15" fill="none" stroke="#fff" strokeWidth="2.7" strokeLinecap="round" />
  </svg>;
}

function AppleNewsIcon() {
  return <svg className="aurora-source-icon aurora-news-icon" aria-hidden="true" viewBox="0 0 32 32">
    <rect width="32" height="32" rx="2.5" fill="#ff375f" />
    <path d="M7 25V7h3.4l11.2 14.1V7H25v18h-3.4L10.4 10.9V25H7Z" fill="#fff" />
  </svg>;
}

function Award({ title, source, icon }: { title: ReactNode; source: string; icon: "store" | "news" }) {
  return <div className="aurora-award">
    <Laurel side="left" />
    <div className="aurora-award-copy"><strong>{title}</strong><small>{icon === "store" ? <AppStoreIcon /> : <AppleNewsIcon />}{source}</small></div>
    <Laurel side="right" />
  </div>;
}
...
function SocialProof({ previous, next }: ScreenProps) { return <section className="aurora-screen aurora-social-proof"><FlowHeader progress={12} previous={previous} close={next} /><h1>Join <Emphasis>4 million</Emphasis> home cooks eating delicious food, every day of the week.</h1><div className="aurora-awards"><Award title={<>4.8 <span className="aurora-stars" aria-label="5 out of 5 stars">★★★★★</span></>} source="App Store" icon="store" /><Award title="App of the Day" source="App Store" icon="store" /><Award title="Recipes featured" source="App News" icon="news" /></div><button className="aurora-bottom-button" onClick={next}>Show me how</button></section>; }
...
function PaymentSuccess({ session }: ScreenProps) { return <section className="aurora-screen aurora-payment-result aurora-payment-success"><span className="aurora-payment-mark" aria-hidden="true">✓</span><p className="aurora-eyebrow">Payment successful</p><h1>Welcome to Aurora.</h1><p>Your payment details are confirmed and your 5-day free trial is active.</p><a className="aurora-payment-button" href={journeyWelcomeUrl(session.sessionId)}>Continue to Aurora <span aria-hidden="true">→</span></a></section>; }
function PaymentFailed({ checkout }: ScreenProps) { const [isCheckingOut, setIsCheckingOut] = useState(false); const [error, setError] = useState(""); const retry = async () => { setError(""); setIsCheckingOut(true); try { await checkout("annual"); } catch { setError("We couldn’t open checkout. Please try again."); setIsCheckingOut(false); } }; return <section className="aurora-screen aurora-payment-result aurora-payment-failed"><span className="aurora-payment-mark" aria-hidden="true">!</span><p className="aurora-eyebrow">Payment unsuccessful</p><h1>Let’s try that again.</h1><p>Your membership has not started. Check your payment details and try again when you’re ready.</p>{error ? <p className="aurora-checkout-error" role="alert">{error}</p> : null}<button className="aurora-payment-button" disabled={isCheckingOut} onClick={() => void retry()}>{isCheckingOut ? "Opening checkout…" : "Try payment again"}</button></section>; }
...
export const auroraMealPlan = defineFunnel({ id: "aurora-meal-plan", productId: funnelProducts["aurora-meal-plan"], domains: ["begin.aurorafirst.ai", "preview-begin.aurorafirst.ai"], defaultOffer: "annual", checkoutOffers: ["annual"], checkoutRoutes: { annual: { paid: "payment-success", trialing: "payment-success", failed: "payment-failed", cancelled: "payment-failed" } }, screens: [
  ...
  defineScreen({ id: "healthy", type: "quiz_question", component: healthy }), defineScreen({ id: "healthy-benefit", type: "content", component: pitch({ progress: 72, visual: "healthy", title: <>Healthy recipes. No compromise on <Emphasis>flavour.</Emphasis></>, body: "Macros on every recipe. Filter by protein, calories, and ingredients to either include or avoid." }) }), defineScreen({ id: "meals", type: "quiz_question", component: meals }), defineScreen({ id: "loader", type: "loader", component: Loader }), defineScreen({ id: "plan-preview", type: "result", component: PlanPreview }), defineScreen({ id: "account", type: "email_capture", component: Account }), defineScreen({ id: "details", type: "quiz_question", component: Details }), defineScreen({ id: "paywall", type: "paywall", component: Paywall, conversion: "subscription_started" }), defineScreen({ id: "payment-success", type: "success", component: PaymentSuccess }), defineScreen({ id: "payment-failed", type: "content", component: PaymentFailed }),
] });

export const funnels: Record<string, FunnelDefinition> = { "aurora-meal-plan": auroraMealPlan };
export const defaultFunnelId = "aurora-meal-plan";
