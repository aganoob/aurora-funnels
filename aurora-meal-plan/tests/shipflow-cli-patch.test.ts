...
const patchFile = new URL("../patches/@aganoob__cli@2.2.2.patch", import.meta.url);
const cloudRunPatchFile = new URL("../patches/@aganoob__deployment-gcp-cloud-run@2.2.0.patch", import.meta.url);

describe("Shipflow CLI patch", () => {
  it("loads ESM deployment adapters and supplies server analytics credentials", async () => {
    ...
    expect(patch).toContain("process.env.META_TEST_EVENT_CODE");
    expect(patch).toContain('name === "NEXT_PUBLIC_APP_URL"');
  });
});
